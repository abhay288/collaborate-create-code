...
serve(async (req: Request) => {
  ...
  try {
    ...
    // SECURITY: userId must NEVER come from client input
    // Fallback to body only if explicitly allowed for server-to-server calls
    if (!userId) {
      console.error('[RecommendationEngine] No authenticated user found');
      return new Response(
    ...
    console.log('[RecommendationEngine] Verified user ID:', userId);
    ...
    const { data: userProfile } = await supabaseAdmin
      .from('profiles')
      .select('class_level, study_area, preferred_state, preferred_district, interests, preferred_language')
      .eq('id', userId)
      .maybeSingle();
...
- Quantitative + Analytical high → CA, CFA, Data Science, Fintech, Actuarial Science
- Creative + Verbal high → B.Des (UI/UX, Product), Mass Comm, Content Strategy
- Interpersonal + Verbal high → MBA, Law, Psychology, HR
...
Generate exactly 5 career/course recommendations ranked by suitability. Include specific courses like B.Tech CSE, MBBS, B.Com, etc. for Indian context.`
    ...
    if (!collegeError && colleges && colleges.length > 0) {
      collegeRecommendations = colleges.map((c: any) => ({
        id: c.id,
      ...
      if (fallbackColleges) {
        collegeRecommendations = fallbackColleges.map((c: any) => ({
          id: c.id,
          name: c.college_name || 'Unknown College',
          state: c.state || 'Unknown',
          district: c.district || null,
    ...
    for (const rec of recommendations) {
      try {
        ...
        let careerId;
        if (!existingCareer) {
          // SECURITY: Insert into pending_careers for admin review instead of directly into careers
          const { error: pendingError } = await supabaseAdmin
            .from('pending_careers')
            .insert({
          ...
          if (pendingError) {
            console.error('[RecommendationEngine] Error creating pending career:', pendingError);
          }
          
          // Create a temporary career entry so recommendation can still be saved
          ...
          if (careerError || !newCareer) {
            console.error('[RecommendationEngine] Error creating career:', careerError);
            continue; // Skip this recommendation instead of crashing
          }
    ...
    // Update profile with individual skill scores
    const profileScores: Record<string, number> = {
      logical_score: scores.logical || scores.logical_reasoning || 0,
      numerical_score: scores.quantitative || scores.numerical || 0,
      verbal_score: scores.verbal || 0,
      technical_score: scores.technical || scores.technical_interests || 0,
      creative_score: scores.creative || scores.creativity || 0,
    ...
    if (profileUpdateError) {
      console.error('[RecommendationEngine] Error updating profile scores:', profileUpdateError);
    } else {
      console.log('[RecommendationEngine] Profile scores updated:', profileScores);
    }
    ...
    console.error('[RecommendationEngine] Error:', error instanceof Error ? error.message : error);
    
    // Sanitize error message for client - don't expose internal details
    let clientMessage = 'Failed to generate recommendations. Please try again later.';
    
    if (error instanceof Error) {
      // Only expose specific user-friendly errors
      if (error.message.includes('Rate limit')) {
        clientMessage = 'Service is busy. Please try again in a few minutes.';
      } else if (error.message.includes('Unauthorized') || error.message.includes('No valid user')) {
        clientMessage = 'Please log in to generate recommendations.';
      } else if (error.message.includes('Invalid quizSessionId')) {
        clientMessage = 'Invalid quiz session. Please restart the quiz.';
      }
    }
