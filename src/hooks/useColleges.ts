  ...
  useEffect(() => {
    const fetchStates = async () => {
      try {
        ...
        // IMPORTANT: Use RPC to avoid PostgREST row limits with large datasets.
        const { data: stateRows, error: stateError } = await supabase
          .rpc('get_distinct_college_states');
        ...
        const allStates = (stateRows || [])
          .map((r: any) => normalizeString(r.state))
          .filter(s => s && s.toLowerCase() !== 'unknown' && s.length > 0)
          .map(s => toTitleCase(s));

        const uniqueStates = [...new Set(allStates)].sort((a, b) => a.localeCompare(b));
  ...
  useEffect(() => {
    const fetchDistricts = async () => {
      try {
        ...
        // Districts are dependent on a selected state.
        ...
        const { data: districtRows, error } = await supabase
          .rpc('get_distinct_college_districts', { p_state: filters.state });
        ...
        const allDistricts = (districtRows || [])
          .map((r: any) => normalizeString(r.district))
